...
void yacc::add_terminate_ids(const std::unordered_set<std::string> &ids) {
  terminate_ids__ = ids;
}
std::string yacc::state_to_string(const std::set<yacc_state_node_t> &state) {
  if (state.size() == 1) {
...
std::vector<std::pair<std::pair<std::string, std::string>, yacc_state_node_t>>
yacc::dfa_optimize(
    const std::vector<
...
std::vector<std::pair<std::pair<std::set<yacc_state_node_t>, std::string>,
  ...
  for (size_t i = 0; i < state.size(); ++i) {
    ...
    for (auto &q : state_backup) {
      for (auto &p : term_words) {
        ...
        const auto conv_find = [&](auto item) { return item.first == conv; };
        auto old_iter = old_machine.end();
        if ((old_iter = std::find_if(old_machine.begin(), old_machine.end(),
                                     conv_find)) != old_machine.end()) {
          new_state.insert(old_iter->second);
          while ((old_iter = std::find_if(old_iter + 1, old_machine.end(),
                                          conv_find)) != old_machine.end()) {
            new_state.insert(old_iter->second);
          }
        }
...
std::vector<
    std::pair<std::pair<yacc_state_node_t, std::string>, yacc_state_node_t>>
...
yacc::make_term_words(const std::vector<yacc_rule> &rules) {
...
bool yacc::self_reduce_two(
    std::vector<std::shared_ptr<yacc_result_t>> &result,
    const std::vector<
        std::pair<std::pair<std::string, std::string>, yacc_state_node_t>> &dfa,
    const std::unordered_set<std::string> &term_words) {
  if (result.size() < 2)
    return false;
